...
Muon::Muon( const Ntuple::MuonNtupleObject* ana, const Int_t idx ) : Basic( idx ), Particle() {
  ...
  */
  ...
  if(  ((ana->detailLevel & Ntuple::MuonBoostedIsolation) == Ntuple::MuonBoostedIsolation)  ) {
    if (ana->m_connectsucceeded[28]) m_SemileptonicPFIso = &((*ana->SemileptonicPFIso)[idx]); else m_SemileptonicPFIso = 0; 
    if (ana->m_connectsucceeded[29]) m_SemileptonicCorrPFIso = &((*ana->SemileptonicCorrPFIso)[idx]); else m_SemileptonicCorrPFIso = 0; 
  ...
  if(  ((ana->detailLevel & Ntuple::MuonIsolation) == Ntuple::MuonIsolation)  ) {
    if (ana->m_connectsucceeded[12]) m_pfDeltaCorrRelIso = &((*ana->pfDeltaCorrRelIso)[idx]); else m_pfDeltaCorrRelIso = 0; 
    if (ana->m_connectsucceeded[13]) m_photonIso = &((*ana->photonIso)[idx]); else m_photonIso = 0; 
    if (ana->m_connectsucceeded[14]) m_neutralHadIso = &((*ana->neutralHadIso)[idx]); else m_neutralHadIso = 0; 
    if (ana->m_connectsucceeded[15]) m_chargedHadIso = &((*ana->chargedHadIso)[idx]); else m_chargedHadIso = 0; 
    if (ana->m_connectsucceeded[16]) m_trackIso = &((*ana->trackIso)[idx]); else m_trackIso = 0; 
  } // end of detail level Isolation
  
  if(  ((ana->detailLevel & Ntuple::MuonTrack) == Ntuple::MuonTrack)  ) {
    if (ana->m_connectsucceeded[17]) m_d0 = &((*ana->d0)[idx]); else m_d0 = 0; 
    if (ana->m_connectsucceeded[18]) m_dz = &((*ana->dz)[idx]); else m_dz = 0; 
    if (ana->m_connectsucceeded[19]) m_d0_allvertices = &((*ana->d0_allvertices)[idx]); else m_d0_allvertices = 0; 
    if (ana->m_connectsucceeded[20]) m_dz_allvertices = &((*ana->dz_allvertices)[idx]); else m_dz_allvertices = 0; 
...
ostream& operator<<( ostream& out, const Muon& rhs ) {
...
bool sortMuonPt::operator()( const Muon& e1, const Muon& e2 ) {
