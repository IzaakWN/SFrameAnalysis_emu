...
Electron::Electron( const Ntuple::ElectronNtupleObject* ana, const Int_t idx ) : Basic( idx ), Particle() {
  ...
  */
  ...
  if(  ((ana->detailLevel & Ntuple::ElectronBoostedID) == Ntuple::ElectronBoostedID)  ) {
    if (ana->m_connectsucceeded[43]) m_isVetoElectronBoosted = &((*ana->isVetoElectronBoosted)[idx]); else m_isVetoElectronBoosted = 0; 
    if (ana->m_connectsucceeded[44]) m_isMediumElectronBoosted = &((*ana->isMediumElectronBoosted)[idx]); else m_isMediumElectronBoosted = 0; 
    if (ana->m_connectsucceeded[45]) m_isTightElectronBoosted = &((*ana->isTightElectronBoosted)[idx]); else m_isTightElectronBoosted = 0; 
    if (ana->m_connectsucceeded[46]) m_isHeepElectronBoosted = &((*ana->isHeepElectronBoosted)[idx]); else m_isHeepElectronBoosted = 0; 
    if (ana->m_connectsucceeded[47]) m_isLooseElectronBoosted = &((*ana->isLooseElectronBoosted)[idx]); else m_isLooseElectronBoosted = 0; 
  } // end of detail level BoostedID
  
  if(  ((ana->detailLevel & Ntuple::ElectronBoostedIsolation) == Ntuple::ElectronBoostedIsolation)  ) {
    if (ana->m_connectsucceeded[48]) m_SemileptonicPFIso = &((*ana->SemileptonicPFIso)[idx]); else m_SemileptonicPFIso = 0; 
    if (ana->m_connectsucceeded[49]) m_SemileptonicCorrPFIso = &((*ana->SemileptonicCorrPFIso)[idx]); else m_SemileptonicCorrPFIso = 0; 
